...
def resnet18(pretrained=False, channel=20, **kwargs):
    ...
    """
    model = ResNet(
        BasicBlock, [2, 2, 2, 2], nb_classes=101, channel=channel, **kwargs)
    if pretrained:
...
def resnet34(pretrained=False, channel=20, **kwargs):
    ...
    model = ResNet(
        BasicBlock, [3, 4, 6, 3], nb_classes=101, channel=channel, **kwargs)
    if pretrained:
        pretrain_dict = model_zoo.load_url(
...
def resnet50(pretrained=False, channel=20, **kwargs):
    ...
    model = ResNet(
        Bottleneck, [3, 4, 6, 3], nb_classes=101, channel=channel, **kwargs)
    if pretrained:
        pretrain_dict = model_zoo.load_url(
...
def weight_transform(model_dict, pretrain_dict, channel):
    ...
    if channel == 3:
        ...
        wt = cross_modality_pretrain(Variable(w3), channel)
