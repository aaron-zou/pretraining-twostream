...
class Spatial_CNN():
    ...
    def build_model(self):
        ...
        self.optimizer = torch.optim.SGD(
            self.model.parameters(), self.lr, momentum=0.9)
    ...
    def train_1epoch(self):
        print(
            '==> Epoch:[{0}/{1}][training stage]'.format(self.epoch, self.nb_epochs))
        ...
        for i, (data_dict, label) in enumerate(progress):
            ...
            # compute output
            output = Variable(torch.zeros(
                len(data_dict['img1']), 101).float()).cuda()
    ...
    def validate_1epoch(self):
        print(
            '==> Epoch:[{0}/{1}][validation stage]'.format(self.epoch, self.nb_epochs))
        batch_time = AverageMeter()
        losses = AverageMeter()
        top1 = AverageMeter()
        top5 = AverageMeter()
