...
class motion_dataset(Dataset):
    ...
    def stackopf(self):
        ...
        flow = torch.FloatTensor(
            2*self.in_channel, self.img_rows, self.img_cols)
...
class Motion_DataLoader():
    ...
    def train(self):
        training_set = motion_dataset(dic=self.dic_video_train,
        ...
        print('==> Training data: {} videos {}'.format(
            len(training_set),  training_set[1][0].size()))
    ...
    def val(self):
        validation_set = motion_dataset(dic=self.dic_test_idx,
                                        in_channel=self.in_channel,
                                        root_dir=self.data_path,
                                        mode='val',
                                        transform=transforms.Compose([
        ...
        print('==> Validation data: {} frames {}'.format(
            len(validation_set), validation_set[1][1].size()))
